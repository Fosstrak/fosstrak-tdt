...
/**
 *
 *	<p>This class provides methods for translating an electronic
 *	product code (EPC) between various levels of representation
 *	including BINARY, TAG_ENCODING, PURE_IDENTITY and LEGACY
 *	formats.  An additional output level ONS_HOSTNAME may be
 *	defined for some coding schemes. When the class TDTEngine is
 ...
 *      <pre><code>TDTEngine engine = new TDTEngine("/opt/TDT");</code></pre></p>
 *      
 ...
 *      subdirectory called <code>schemes</code>.  Any auxiliary lookup files
...
public class TDTEngine {
    ...
    /** 
     * prefix_tree_map is a map of levels to prefix trees. Each prefix
     * tree is a Trie structure (see wikipedia) that is useful for
     * quickly finding a matching prefix. 
     */
    private Map<LevelTypeList,PrefixTree<PrefixMatch>> prefix_tree_map = new HashMap<LevelTypeList,PrefixTree<PrefixMatch>>();
	
    /**
     *	HashMap gs1cpi
     *	is an associative array providing a lookup between either a GS1 Company Prefix and the corresponding integer-based Company Prefix Index, where
     *	one of these has been registered for use with 64-bit EPCs - or the reverse lookup from Company Prefix Index to GS1 Company Prefix.
     *	Note that this is an optimization to avoid having to do an xpath trawl through the CPI table each time.
     */
    private HashMap<String,String> gs1cpi = new HashMap<String,String>();	

    /**
     *	HashMap levelnumber
     *	provides a lookup between a string presentation of a level and the corresponding numeric level 0-4
     */
    //    private HashMap<String,Integer> levelnumber = new HashMap<String,Integer>();

    /**
     *	String xmldir A private copy of the directory name which was
     *	specified when the class TDTEngine was constructed
     */
     ...
     *  <code><pre>TDTEngine engine = new TDTEngine("/opt/TDT");</pre></code></p>
     *  <p>The
     *  specified directory must contain two subdirectories named
     *  auxiliary and schemes.  The Tag Data Translation definition
     *  files for the various coding schemes should be located
     *  inside the subdirectory called <code>schemes</code>.  Any auxiliary
     ...
     *
     *  Files within the schemes directory ending in <code>.xml</code> are
     *  read in and unmarshalled using <a 
     *  href = "http://www.castor.org">Castor</a>. 
     */
	...
	File [] dir = new java.io.File(confdir+File.separator+"schemes").listFiles(new XMLFilenameFilter());
	//java.util.Arrays.sort(dir); // Sort it
	if (dir == null) 
	    throw new TDTException("Cannot find schemes in " + confdir);
	Unmarshaller unmar = new Unmarshaller();
	for (File f : dir) { 
	    EpcTagDataTranslation tdt = (EpcTagDataTranslation) 
		unmar.unmarshal(EpcTagDataTranslation.class, 
				new FileReader(f));
	...
	//Unmarshaller unmar = new Unmarshaller();
	GEPC64Table cpilookup = (GEPC64Table) 
	    unmar.unmarshal(GEPC64Table.class, 
			    new FileReader(confdir+File.separator+"auxiliary"+File.separator+"ManagerTranslation.xml"));
	...
	public Scheme getScheme() { return s; } 
    ...
    /** 
     *     Given an input string, and optionally a tag length, find a
     *     scheme / level with a matching prefix and tag length.
     */
    ...
    /** 
     *     Given an input string, level, and optionally a tag length,
    ...
    /** 
     *	Translates the input string of a specified input level to a
     *	specified outbound level of the same coding scheme.  For
     *	example, the input string value may be a tag-encoding URI and
     *	the outbound level specified by string outboundlevel may be
     *	BINARY, in which case the return value is a binary
     *	representation expressed as a string.
     * 
     *  <p>Note that this version of the method requires that the user
     *  specify the input level, rather than searching for it. However
     *  it still automatically finds the scheme used.</p>
     *
    ...
    /**
     *	The convert method translates a String input to a specified
     *	outbound level of the same coding scheme.  For example, the
     *	input string value may be a tag-encoding URI and the outbound
     *	level specified by string outboundlevel may be BINARY, in
     *	which case the return value is a binary representation
     *	expressed as a string.
     *	@param input the identifier to be converted.
     *	@param inputParameters additional parameters which need to be
     *	provided because they cannot always be determined from the
     *	input value alone.  Examples include the taglength,
     *	companyprefixlength and filter values.
     *	@param outputLevel the outbound level required for the ouput.
     *	Permitted values include BINARY, TAG_ENCODING, PURE_IDENTITY,
     *	LEGACY and ONS_HOSTNAME.
     ...
     */
    private String convertLevel(Scheme tdtscheme, 
				Level tdtlevel,
				String input, 
				Map<String, String> inputParameters, 
	...
	for (Enumeration e = matchingOption.enumerateField(); e.hasMoreElements();) { 
		...
		if (field.getCharacterSet() != null) { // if the character set is specified
			...
			throw new TDTException("field "+strfieldname+" ("+strfieldvalue+") does not conform to the allowed character set ("+field.getCharacterSet()+") ");
		    }
		}
		...
		if (field.getDecimalMinimum() != null) { // if the decimal minimum is specified
		    ...
		    if (bigvalue.compareTo(bigmin) == -1) { // throw an exception if the field value is less than the decimal minimum
			throw new TDTException("field "+strfieldname+" ("+bigvalue+") is less than DecimalMinimum ("+field.getDecimalMinimum()+") allowed");
		    }
		}
		...
		if (field.getDecimalMaximum() != null) { // if the decimal maximum is specified
		    ...
		    if (bigvalue.compareTo(bigmax) == 1) { // throw an excpetion if the field value is greater than the decimal maximum
			throw new TDTException("field "+strfieldname+" ("+bigvalue+") is greater than DecimalMaximum ("+field.getDecimalMaximum()+") allowed");
		    }
		}
		...
		// compaction is specified - interpret binary as a string value using a truncated byte per character
	...
	/**	the EXTRACT rules are performed after parsing the
	 *	input, in order to determine additional fields that
	 *	are to be derived from the fields obtained by the
	...
	/**     Now we need to consider the corresponding output level
	 *	and output option.  The scheme must remain the same,
	 *	as must the value of optionkey (to select the
	 *	corresponding option element nested within the
	 *	required outbound level)
	 */
						
	Level tdtoutlevel = findLevel(tdtscheme, outboundlevel);
	Option tdtoutoption = findOption(tdtoutlevel, optionValue);
			
	/**	the FORMAT rules are performed before formatting the
	 *	output, in order to determine additional fields that
	 *	are required for preparation of the outbound format
	 */
	...
	/**     Now we need to ensure that all fields required for the
	 *      outbound grammar are suitably padded etc.
	 *      processPadding takes care of firstly padding the
	 *      non-binary fields if padChar and padDir, length are
	 *      specified then (if necessary) converting to binary and
	 *      padding the binary representation to the left with
	 *      zeros if the bit string is has fewer bits than the
	 *      bitLength attribute specifies.  N.B. TDTv1.1 will be
	 *      more specific about bit-level padding rather than
	 *      assuming that it is always to the left with the zero
	 *      bit.
	 */
	...
	/**    Construct the output from the specified grammar (in
	 *     ABNF format) together with the field values stored in
	 *     inputparams
	 */
    ...
    /**
     *	Returns a string built using a particular grammar.
     *	Single-quotes strings are counted as literal strings, whereas
     *	all other strings appearing in the grammar require
     *	substitution with the corresponding value from the extraparams
     *	hashmap.
     */
    ...
    /**
     *	
     *	Converts the value of a specified fieldname from the
     *	extraparams map into binary, either handling it as a large
     *	integer or taking into account the compaction of each ASCII
     *	byte that is specified in the TDT definition file for that
     *	particular field
     */
	...
	*/
    ...
    /**
     *    If the outbound level is BINARY, convert the string field to
     *    binary, then pad to the left with the appropriate number of
     *    zero bits to reach a number of bits specified by the
     *    bitLength attribute of the TDT definition file.
     */
    ...
    /**
     *	Removes leading or trailing characters equal to padchar from
     *	the start/end of the string specified as the first parameter.
     *	The second parameter specified the stripping direction as
     *	"LEFT" or "RIGHT" and the third parameter specifies the
	...
	if (paddir == null || padchar == null)
		...
		pattern = "["+padchar+"]+$";
	    else //if (paddir == PadDirectionList.LEFT) 
		pattern = "^["+padchar+"]+";
    ...
    /**
     *	
     *	Adds additional entries to the extraparams hashmap by
     *	processing various rules defined in the TDT definition files.
     *	Typically used for string processing functions, lookup in
     *	tables, calculation of check digits etc.
     */
	...
	/**
	 *	Stores in the hashmap extraparams the value obtained
	 *	from a lookup in a specified XML table.
	 *
	 *	The first parameter is the given value already known.
	 *	This is denoted as $1 in the corresponding XPath
	 *	expression
	 *
	 *	The second parameter is the string filename of the
	 *	table which must be present in the auxiliary subdirectory
	 *
	 *	The third parameter is the column in which the
	 *	supplied input value should be sought
	 *
	 *	The fourth parameter is the column whose value should
	 *	be read for the corresponding row, in order to obtain
	 *	the result of the lookup.
	 *
	 *	The rule in the definition file may contain an XPath
	 *	expression and a URL where the table may be obtained.
	 */
	...
	/**
	 *	Stores the length of the specified string under the
	 *	new fieldname specified by the corresponding rule of
	 *	the definition file.
	 */
	...
	/**
	 *	Stores a GS1 check digit in the extraparams hashmap, 
	 *	keyed under the new fieldname specified by the
	...
	/**
	 *	Obtains a substring of the string provided as the
	 *	first parameter.  If only a single second parameter is
	 *	specified, then this is considered as the start index
	 *	and all characters from the start index onwards are
	 *	stored in the extraparams hashmap under the key named
	 *	'newfieldname' in the corresponding rule of the
	 *	definition file.  If a second and third parameter are
	 *	specified, then the second parameter is the start
	 *	index and the third is the length of characters
	 *	required.  A substring consisting characters from the
	 *	start index up to the required length of characters is
	 *	stored in the extraparams hashmap, keyed under the new
	 *	fieldname specified by the corresponding rule of the
	 *	defintion file.
	 */
	if (rulename.equals("SUBSTR")) {
	    ...
	    if (parameter.length == 3) { // need to check that this variation is correct - c.f. Perl substr
		assert extraparams.get(parameter[0]) != null : tdtfunction + " when " + parameter[0] + " is null";
		if (extraparams.get(parameter[0]) != null) {
		    int start = getIntValue(parameter[1], extraparams);
	...
	/**
	 *	Concatenates specified string parameters together.
	 *	Literal values must be enclosed within single or
	 *	double quotes or consist of unquoted digits. Other
	 *	unquoted strings are considered as fieldnames and the
	 *	corresponding value from the extraparams hashmap are
	 *	inserted.  The result of the concatenation (and
	 *	substitution) of the strings is stored as a new entry
	 *	in the extraparams hashmap, keyed under the new
    ...
    /**
     *	
     *	Returns the value of a specified fieldname from the specified
     *	hashmap and returns an integer value or throws an exception if
    ...
    /**
     *	
     *	Performs an XPATH lookup in an xml document whose filename is
     *	specified by the first parameter (assumed to be in the auxiliary
     *	subdirectory) The XPATH expression is supplied as the second
     *	string parameter The return value is of type string e.g. this
     *	is currently used primarily for looking up the Company Prefix
     *	Index for encoding a GS1 Company Prefix into a 64-bit EPC tag
     */
    ...
    /**
     *	
     *	Converts a binary string input into a byte string, using
    ...
    /**
     *	
     *	Converts a byte string input into a binary string, using
     *	8-bits per character byte
     */
    ...
    /**
     *	
     *	Converts an ASCII string input into a binary string, using
     *	7-bit compaction of each ASCII byte
     */
    ...
    /**
     *	
     *	Converts a binary string input into an ASCII string output,
     *	assuming that 7-bit compaction was used
     */
     ...
     *	using 6-bit compaction of each ASCII byte
     */
    ...
    /**
     *	
     *	Converts a binary string input into a character string output,
     *	assuming that 6-bit compaction was used
     */
    ...
    /**
     *	Converts an upper case character string input into a binary
     *	string, using 5-bit compaction of each ASCII byte
     */
     ...
     *	
     *	Converts a binary string input into a character string output,
     *	assuming that 5-bit compaction was used
     */
    ...
    /**
     *	Pads a binary value supplied as a string first parameter to
     *	the left with leading zeros in order to reach a required
     *	number of bits, as expressed by the second parameter, reqlen.
     *	Returns a string value corresponding to the binary value left
     *	padded to the required number of bits.
     */
    ...
    /**
     *	Calculates the check digit for a supplied input string
     *	(assuming that the check digit will be the digit immediately
     *	following the supplied input string).  GS1 (formerly EAN.UCC)
     *	check digit calculation methods are used.
     */
    ...
    /**
     * find a level by its type in a scheme.  This involves iterating
     * through the list of levels.  The main reason for doing it this
     * way is to avoid being dependent on the order in which the
     * levels are coded in the xml, which is not explicitly
     * constrained.
     */
    ...
    /**
     * find a option by its type in a scheme.  This involves iterating
     * through the list of options.  The main reason for doing it this
     * way is to avoid being dependent on the order in which the
     * options are coded in the xml, which is not explicitly
     * constrained.
     */
